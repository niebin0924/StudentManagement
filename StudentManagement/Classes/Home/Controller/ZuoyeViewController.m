...
@interface ZuoyeViewController () <UITableViewDataSource,UITableViewDelegate>
{
    NSInteger _currPage;
}
...
- (void)request
{
    ...
    NSDictionary *parameters = @{@"students_id":[[AppDefaultUtil sharedInstance] getStudentId], @"page":[NSString stringWithFormat:@"%zd",_currPage], @"page_size":@"20", @"looked_s":@"1"};
        ...
        if (data) {
            ...
            if ([[dict objectForKey:@"code"] integerValue] == 0) {
                ...
                if (results && results.count > 0) {
                    ...
                    [SVProgressHUD showSuccessWithStatus:@"无作业"];
                }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MessageList *modal = self.dataArray[indexPath.row-1];
    ZuoyeDetailViewController *vc = [[ZuoyeDetailViewController alloc] init];
